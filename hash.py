...
def info():
    ...
    print(Y+"\n 0{======================"+W +
          " INFO "+Y+"=======================}0")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Name     "+C+":" +
          W+" Hasher"+Y+"                               |")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Code     "+C+":" +
          W+" python"+Y+"                               |")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Version  "+C+":" +
          W+" 3.0.1 (alpha)"+Y+"                        |")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Author   "+C+":"+W +
          " Ci Ku "+R+"a.k.a "+W+"Xnver404"+Y+"                 |")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Email    "+C+":" +
          W+" XnVer404@gmail.com"+Y+"                   |")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Github   "+C+":" +
          W+" https://github.com/ciku370"+Y+"           |")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Date     "+C+":" +
          W+" 23 - 02 - 2018"+Y+"                       |")
    print(Y+" |"+B+" ["+R+"="+B+"] "+W+"Team     "+C+":" +
          W+" Blackhole Security"+Y+"                   |")
    print(Y+" 0{===================================================}0\n")
    print(B+" ["+R+"="+B+"] "+W+"python2 "+sys.argv[0] +
          " -u           to update wordlist "+B+"["+R+"="+B+"]")
    print(B+"\n ["+R+"="+B+"] "+W+"list of supported hashes :" +
          Y+" ["+W+"01"+Y+"] "+C+"md4")
...
def Update():
    if sys.platform == "linux" or sys.platform == "linux2":
        ...
        os.system(
            "curl https://raw.githubusercontent.com/CiKu370/hasher/master/wordlist.txt -o wordlist.txt")
...
try:
    ...
    banner()
    time.sleep(0.5)
    print(B+"["+W+"="+B+"] "+G+"installing module " +
          R+"progressbar , passlib\n"+W)
...
#		time.sleep(0.3)
        cek = input(B+"["+W+"?"+B+"] "+G+"Choose hash "+Y+">>> "+W)
...
#				time.sleep(0.3)
                ...
                html = urlopen(
                    "http://md5.my-addr.com/md5_decrypt-md5_cracker_online/md5_decoder_tool.php", data)
                find = html.read()
                match = search(
...
#	                                time.sleep(0.3)
                    ...
                    print(B+"["+W+"+"+B+"] "+W+(hash_str)+Y +
                          " 0={==> "+W+(match.group().split('span')[2][3:-6])+"\n")
                    sys.exit()
                else:
                    data = urlencode({"md5": hash_str, "x": "21", "y": "8"})
                    html = urlopen(
...
#						time.sleep(0.3)
                        print(B+"\n["+W+"="+B+"]"+G+" password found ")
                        print(B+" ["+W+"+"+B+"] "+W+hash_str+Y +
                              " 0={==> "+W+match.group().split('span')[2][3:-6]+W+" \n")
                        sys.exit()
                    else:
...
#			time.sleep(0.3)
            ...
            try:
                ...
                data = urlencode(
                    {"auth": "8272hgt", "hash": hash_str, "string": "", "Submit": "Submit"})
...
#					time.sleep(0.3)
                    print(B+"\n["+W+"="+B+"]"+G+" password found ")
                    print(B+"["+W+"+"+B+"] "+W+hash_str+Y +
                          " 0={==> "+W+match.group().split('hervorheb2>')[1][:-18]+"\n")
                    sys.exit()
...
#                                time.sleep(0.3)
        ...
        for line in pbar(w):
            line = line.strip()
            h = ntlm_hash = binascii.hexlify(hashlib.new(
                'md4', line.encode('utf-16le')).digest())
            if h == hash_str:
